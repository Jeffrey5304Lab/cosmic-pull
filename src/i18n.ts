...
const HINT_EN: Record<number, string> = {
  ...
  7: "The fountain is a machine now: clear one side to fill its cup and open the gate",
  8: "Solve the shelf side first — the gate only then lets the other side fall",
  9: "Two stages: cascade bottom-up, then release the stack behind the gate",
  ...
  12: "Forget the colours for a moment — clear the shelf side first",
  13: "Clear one side bottom-up to fill its cup — only then does the gate open for the other side",
  14: "Both shelves are loaded — release the lower one first or it gets buried",
  15: "Three shelves and a gate: fill one cup first to open the way for the other",
  16: "Clear the ramp first so each colour drops on its own side",
  17: "A two-stage machine — solve the shelf side first, then the gate lets the rest through",
  ...
  20: "Finale machine: cascade bottom-up, fill the cup to open the gate, then release the chained stack",
  ...
  27: "Two halves: do the shelves first, then the gate opens for the other",
  28: "Two zones: one is a bottom-up cascade, the other waits for the gate",
  29: 'Two shelves stacked between lava — release the lower one first, or it spills',
  30: "Two halves — finish the shelf half first",
  31: "A ramp plus roaming lava — deal with the ramp first",
  32: "Two rivers split by a gate — fill one side first",
  33: "The gate needs its key cup full; get the order wrong and you run out of stardust",
  34: "A black hole roams below, and the ramp sends everything one way — deal with the ramp first",
  35: "A chain and a gate in two stages — one wrong step and it cannot be recovered",
